...
public class PipeWriterService implements WriterService {
	...
	public void writeData(List<EmployeeEntity> employeeList) {
		...
		final Logger logger = LoggerFactory.getLogger(PipeWriterService.class);
		if (employeeList.isEmpty()) {
			logger.info("LIST IS EMPTY ...PIPE");
		} else {
			try {
...
//				SimpleDateFormat sdf = new SimpleDateFormat("MMM dd,yyyy HH:mm:ss");  
				...
				FileWriter filew = new FileWriter("files/PIPEFile" + format + ".csv");
				...
				File file = new File("files/PIPEFile" + format + ".csv");
